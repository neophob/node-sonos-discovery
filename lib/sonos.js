...
function Discovery(settings) {
  ...
  function init(info) {
    if (settings.household && settings.household !== info.household) return;

    // We try to subscribe to the first unit we find
    trySubscribe(info.location, info.ip);

    _this.ssdp.stop();
    _this.ssdp.removeListener('found', init);
  }

  // search periodcaly
  this.ssdp.start();
  this.ssdp.on('found', init);
  ...
  function trySubscribe(deviceDescription, address) {
    ...
    _this.urlInfo = url.parse(deviceDescription);

    // The local endpoint needs to be set before subscribing.
    _this.updateLocalEndpoint()
      .then(function() {
...
Discovery.prototype.updateLocalEndpoint = function () {
  ...
  return _this.getLocalEndpoint()
    ...
    .catch(function(error) {
      _this.log.error('Unable to update local endpoint', error);
    });
};

Discovery.prototype.getLocalEndpoint = function() {
  var _this = this;
