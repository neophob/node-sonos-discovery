...
var SSDP = require('./sonos-ssdp.js');
...
function Discovery(settings) {
  ...
  this.household = settings.household;

  this.ssdp = new SSDP({log: this.log });
  ...
  this.updatingEndpointPromise = undefined;

  this.ssdp.discoverPlayer({household: this.household})
    .then(function(playerInfo) {
      // We try to subscribe to the first unit we find
      trySubscribe(playerInfo);
    })
    .catch(function(error) {
      _this.log.error('Unable to find Sonos Topology', error);
      // TODO this would be a good player to emit an error so the subscriber
      // at least knows the discovery failed to start.
    });
  ...
  function trySubscribe(playerInfo) {
    if (_this.subscribedTo !== undefined) {
      return;
    }

    _this.log.info('subscribing to topology', playerInfo.ip);

    _this.subscribedTo = playerInfo.ip;

    _this.urlInfo = url.parse(playerInfo.location);

    // The local endpoint needs to be set before subscribing.
    _this._getLocalEndpoint()
      .then(function(localEndpoint) {
        // We want to know our endpoint IP to expose the correct event url
        // In case of multiple interfaces!
        _this._updateLocalEndpoint(localEndpoint);

        // We don't need anything more, subscribe
        subscribe('/ZoneGroupTopology/Event');
      })
      .catch(function(e) {
        _this.log.error('Unable to initialize local endpoint', e);
        // TODO we could emit an error here, but this will throw
        // an exception if no error handler is defined. (public api change)
      });
  }
  ...
  function subscribe(path) {
    ...
    client.on('error', function (e) {
      ...
      _this.log.error('Topology Player Connection Lost', e);

      // TODO should call a time out to resubscribe here as well.
    });
...
Discovery.prototype.updateLocalEndpoint = function () {
  var _this = this;

  if (_this.updatingEndpointPromise) {
    return _this.updatingEndpointPromise;
  }

  _this.log.debug('discovery: updating local endpoint...', {
    currentEndpoint: _this.localEndpoint,
    currentTopology: _this.urlInfo.hostname
  });

  // We start with the player, because if subscribedTo is wrong
  // _getLocalEndpoint will timeout slowly, after which we would need to update
  // subscribedTo and try _getLocalEndpoint again.
  _this.updatingEndpointPromise = _this.ssdp.discoverPlayer(_this.household)
    .then(_this._updatePlayerSubscription.bind(_this))
    .then(_this._getLocalEndpoint.bind(_this))
    .then(_this._updateLocalEndpoint.bind(_this))
    .catch(function(error) {
      _this.log.error('error updating endpoint', error);
      return Promise.reject(error);
    })
    .finally(function() {
      _this.updatingEndpointPromise = undefined;
    });

  return _this.updatingEndpointPromise;
};

Discovery.prototype._updatePlayerSubscription = function(playerInfo) {
  var _this = this;

  if (_this.subscribedTo !== playerInfo.ip) {
    // TODO here if we have the old uuid we need to update the topology
    // it's possible that it changed.
    _this.log.info('discovery: found new topology player', playerInfo.ip);
    _this.subscribedTo = playerInfo.ip;
    _this.urlInfo = url.parse(playerInfo.location);
  }
}

Discovery.prototype._updateLocalEndpoint = function(localEndpoint) {
  var _this = this;

  if (localEndpoint !== _this.localEndpoint) {
    _this.localEndpoint = localEndpoint;
    _this.log.info('discovery: setting local endpoint', _this.localEndpoint);
  }
};

Discovery.prototype._getLocalEndpoint = function() {
  var _this = this;

  return new Promise(function (resolve, reject) {
    var options = {
      hostname: _this.urlInfo.hostname,
      path: _this.urlInfo.path,
      port: _this.urlInfo.port,
      headers: {
        TIMEOUT: 'Second-6'
      }
    };
    ...
    var request = http.get(options, function (res) {
      var localEndpoint = res.socket.address().address;
