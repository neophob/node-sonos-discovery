...
function Player(roomName, descriptorUrl, uuid, discovery) {
  ...
  function relocatePlayer() {
    ...
    _this.log.info('Attempting to locate SonosPlayer again...');
  ...
  function handleSonosConnectionError(error) {
    ...
    if (possibleLocalIPChangeError) {
      _this.log.info('wrong address local IP could be have changed', {
        IP: _this.address,
    ...
    _this.log.info('Error player could be off or on different IP', {
      IP: _this.address,
      localEndpoint: _this.discovery.address
    });
