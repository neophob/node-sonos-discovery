...
function parseMetaDataXML(xml) {
  ...
  saxParser.on('startNode', function (elem, attr) {
    ...
    if (elem === 'DIDL-Lite') {
      DIDL = '<DIDL-Lite ' + getAttrString(attr()) + '>';
    } else if (elem === 'item' ) {
      currentItemMeta += DIDL + '<item ' + getAttrString(attr()) + '>';
    } else {
      currentItemMeta += '<' + elem + ' ' + getAttrString(attr()) + '>';
    }
  });

  saxParser.on('endNode', function (elem) {
    currentItemMeta += '</' + elem + '>';

    if (elem === 'item') {
      currentItemMeta += '</DIDL-Lite>';

      if (!currentItem.metaData) {
        currentItem.metaData = currentItemMeta;
      }
    ...
    if (mapping[elem] === 'albumArtURI') {
      ...
      } else if (nodeValue.startsWith('http')) {
        currentItem.absoluteAlbumArtURI = nodeValue;
      }
    }
...
function Player(roomName, descriptorUrl, uuid, discovery) {
  discovery.log.debug('Initializing player', roomName);
  var _this = this;
  ...
  this.streamPrefixStrings = ['x-sonosapi-stream:', 'x-sonosapi-radio:', 'pndrradio:', 'x-sonosapi-hls:', 'x-sonosprog-http:'];
  ...
  function updateTransportState(data) {
    var transportMeta = {};
    var streamChecks = _this.streamPrefixStrings.map(function(streamPrefix) { return new RegExp('^'+streamPrefix); });
    var isRadio = false;
    var isLineIn = false;
    ...
    isRadio = streamChecks.some(function(regex) { return regex.test(_this.avTransportUri); });
    isLineIn = isUriLineIn(_this.avTransportUri);
    ...
    } else if (data.CurrentTrackMetaData === '') {
      _this.state.currentTrack.title = '';
      _this.state.currentTrack.album = '';
      _this.state.currentTrack.artist = '';
      _this.state.currentTrack.albumArtURI = '';
      _this.state.currentTrack.absoluteAlbumArtURI = '';
      _this.state.currentTrack.type = 'track';
      _this.state.nextTrack.title = '';
      _this.state.nextTrack.album = '';
      _this.state.nextTrack.artist = '';
      _this.state.nextTrack.albumArtURI = '';
      _this.state.nextTrack.absoluteAlbumArtURI = '';
      _this.state.nextTrack.type = 'track';
    }
  ...
  function getOrInitializeSubscription(path) {
    var subscription;
  ...
  function resubscribeAfterTimeout(path, timeout) {
    var subscription = getOrInitializeSubscription(path);
  ...
  let relocatingPlayerPromsie;

  function relocatePlayer() {
    if (relocatingPlayerPromsie) {
      return relocatingPlayerPromsie;
    }

    relocatingPlayerPromsie = _this.ssdp.discoverPlayer({player: _this.uuid})
      .then(function(playerInfo) {
        return playerInfo;
      })
      .finally(function() {
        // next call to relocatePlayer() should start a new search.
        relocatingPlayerPromsie = undefined;
      });

    return relocatingPlayerPromsie;
  }

  function updatePlayerAddress(playerInfo) {
    if (playerInfo.ip !== _this.address) {
      _this.log.info('SonosPlayer address changed', playerInfo.uuid, playerInfo.ip);
    }

    _this.address = playerInfo.ip;

    return playerInfo;
  }

  function handleSonosConnectionError(error) {
    let possibleLocalIPChangeError = error.code === 'EADDRNOTAVAIL';

    if (possibleLocalIPChangeError) {
      return discovery.updateLocalEndpoint();
    }

    return relocatePlayer().then(updatePlayerAddress);
  }

  _this.handleSonosConnectionError = handleSonosConnectionError;
  ...
  function subscribe(path) {
    ...
    var client = http.request({
      ...
      if (res.statusCode == 200) {
        ...
        resubscribeAfterTimeout(path, subscriptionTimeout * 500);
      } else {
        ...
        resubscribeAfterTimeout(path, 5000);
      }
    ...
    client.on('error', function (e) {
      ...
      handleSonosConnectionError(e)
        .catch(function(error) {
          // Couldn't find a better way to check for {}
          // It looks like even though recovery succeded
          // it can get an empty object.
          if (error.code) {
            _this.log.error('Connection Recovery Error', error, path);
          }
        })
        .finally(function() {
          // Keep trying...
          delete subscription.sid;
          resubscribeAfterTimeout(path, 10000);
        })
    });
...
Player.prototype.soapAction = function (path, action, soap, callback) {
  ...
  req.on('error', function (soapError) {
    _this.log.error('soapAction error', soapError.message);

    let oldIp = _this.address;

    // Try to renew the player/subscriber ips
    _this.handleSonosConnectionError(soapError)
      .catch(function(error) {
        _this.log.error('Recovery Error', error, path);

        if (!callback) return;
        callback(soapError.message);
      })
      .finally(function() {
        if (oldIp !== _this.address) {
          _this.log.info('Sonos addresses updated', {
            sonosIp: _this.address,
            localEndpoint: _this.discovery.address
          });
        }
      });
  });
...
Player.prototype.setVolume = function (volumeLevel, callback) {
  ...
  // If prefixed with + or -
  if (/^[+\-]/.test(volumeLevel)) {
    volumeLevel = parseInt(this.state.volume, 10) + parseInt(volumeLevel, 10);
  }
...
Player.prototype.groupSetVolume = function (volumeLevel, callback) {
  var actions = [];
  var count = 0;
  var groupVolume = parseInt(this.groupState.volume, 10);

  var deltaVolume;

  // If prefixed with + or -
  if (/^[+-]/.test(volumeLevel)) {
    deltaVolume = parseInt(volumeLevel, 10);
    volumeLevel = groupVolume + parseInt(volumeLevel);
  } else {
    volumeLevel = parseInt(volumeLevel, 10);
  ...
  for (var uuid in this.discovery.players) {
    ...
    if (volumeLevel < 1)
      ...
      newVolume = parseInt(player.state.volume, 10) + deltaVolume;
    else {
      var factor = parseInt(player.state.volume) / groupVolume;
  ...
  _this.getFavorites(function (error, favorites) {
    ...
    favorites.forEach(function (item) {
      if (item.title.toLowerCase() == decodeURIComponent(favorite).toLowerCase()) {
        ...
        if (_this.streamPrefixStrings.some(function(item) { return item.uri.startsWith(item); })) {
