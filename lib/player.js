...
function Player(roomName, descriptorUrl, uuid, discovery) {
  ...
  let onGoingRelocatePromise;

  function relocatePlayer() {
    if (onGoingRelocatePromise) {
      return onGoingRelocatePromise;
    }

    onGoingRelocatePromise = new Promise(function(resolve) {
      _this.log.info('Attempting to locate SonosPlayer again...');

      let sonosFound = function(info) {
        // Skip if we found a different player.
        if (info.uuid !== _this.uuid) return;

        _this.ssdp.stop();
        _this.ssdp.removeListener('found', sonosFound);

        onGoingRelocatePromise = undefined;
        resolve(info);
      };

      _this.ssdp.on('found', sonosFound);
      _this.ssdp.start();
    });
  ...
  function updatePlayerAddress(playerInfo) {
    _this.log.info('SonosPlayer address changed', playerInfo.uuid, playerInfo.address);
  ...
  function subscribe(path) {
    ...
    client.on('error', function (e) {
      ...
      var errorHandlersPromise = Promise.resolve();

      if (e.code === 'EADDRNOTAVAIL') {
        errorHandlersPromise = discovery.updateLocalEndpoint();
      }
      else if (e.code === 'ETIMEDOUT') {
        _this.log.info('Timeout error player could be off or on different IP',  subscription.sid, path)
        errorHandlersPromise = relocatePlayer()
          .then(updatePlayerAddress);
      }

      errorHandlersPromise.finally(function() {
        // Keep trying...
        delete subscription.sid;
