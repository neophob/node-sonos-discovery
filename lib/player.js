...
function parseMetaDataXML(xml, playlistWorkAround) {
  var result = [];
  ...
  saxParser.on('textNode', function (str, uq) {
    nodeValue = uq(str);
    if (playlistWorkAround && nodeValue !== undefined) {
      rawNode += nodeValue;
    }
  });

  saxParser.on('startNode', function (elem, attr, uq, tagend, getStrNodeFn) {
    nodeValue = '';
    if (playlistWorkAround) {
      var rawXml = uq(getStrNodeFn());
      if (elem === 'DIDL-Lite') {
        didlHeader = rawXml;
      } else
      if (elem === 'item') {
        rawNode = didlHeader + rawXml;
      } else {
        rawNode += rawXml;
      }
    }
  ...
  saxParser.on('endNode', function (elem, uq, tagstart, getStrNodeFn) {
    if (playlistWorkAround) {
      rawNode += uq(getStrNodeFn());
    }
    if (elem === 'container' || elem === 'item') {
      if (playlistWorkAround) {
        if (currentItem.metaData === undefined) {
          //TODO: the closing </DIDL-Lite> is missing
          currentItem.metaData = rawNode;
        }
      }
...
Player.prototype.browse = function (objectID, startIndex, requestedCount, playlistWorkaround, callback) {
  var _this = this;
  ...
  this.soapAction('/MediaServer/ContentDirectory/Control', '"urn:schemas-upnp-org:service:ContentDirectory:1#Browse"', soap, function (error, res) {
    ...
    res.on('end', function () {
      ...
      response.items = parseMetaDataXML(metaDataXML, playlistWorkaround);
...
Player.prototype.getFavorites = function (callback) {
  this.browse('FV:2', null, null, false, function (error, result) {
    if (error) {
      callback(error);
      return;
    }
...
Player.prototype.getPlaylists = function (callback) {
  this.browse('SQ:', null, null, true, function (error, result) {
    if (error) {
      callback(error);
      return;
    }
...
Player.prototype.getQueue = function (startIndex, requestedCount, callback) {
  this.browse('Q:0', startIndex, requestedCount, false, callback);
};
